...
| glm-5.3-flash   | 本地（不限量）         | ✅ `-thinking` 可用   | 0.198s            | 403.89 tokens/s |
| qwen-3.8        | 本地（不限量）         | ❌ 无思考模式          | 0.214s            | 411.43 tokens/s |
  ...
  "provider": {
    "genai": {
      ...
      "models": {
        ...
        "glm-5.3-flash":          { "name": "GLM 5.3 Flash",       "tool_call": true, "attachment": true, "limit": { "context": 262144, "output": 16384 } },
        "glm-5.3-flash-search":   { "name": "GLM 5.3 Flash (联网)","tool_call": true, "attachment": true, "limit": { "context": 262144, "output": 16384 } },
        "qwen-3.8":               { "name": "Qwen 3.8",            "tool_call": true, "attachment": true, "limit": { "context": 131072, "output": 16384 } },
        "qwen-3.8-search":        { "name": "Qwen 3.8 (联网)",     "tool_call": true, "attachment": true, "limit": { "context": 131072, "output": 16384 } },
...
- `attachment: true` 的模型支持在对话中贴图（本地图以 data URL 上送，代理自动转 GenAI 图片服务；2026-09-26 全部 8 个模型视觉实测通过）。注意 GLM 带图首 token 偏慢且偶发超时，已在代理侧把上游超时放宽到 120s。
...
- 已知不兼容：deepseek-v4.1 的 `-thinking` 与工具调用不能同时用（上游会只输出思维链不输出正文/调用），代理对带工具的 deepseek 请求自动丢弃 thinking 标志。
